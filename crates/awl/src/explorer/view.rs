...
pub fn draw_explorer(buf: &mut Buffer, app: &App, layout: &Layout) {
    ...
    buf.write_str(hx, layout.explorer.y, root_glyph, icons::color(&root_name, true), bg_dark());
    hx += 3;
    ...
    for abs_i in app.explorer_scroll..end {
        ...
        let is_sel = abs_i == app.explorer_selected || (!app.explorer_selection.is_empty() && app.explorer_selection.contains(&abs_i));
