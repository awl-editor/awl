...
fn parse_hex(s: &str) -> Result<Color, String> {
    ...
    let r = u8::from_str_radix(&s[0..2], 16)
        .map_err(|_| format!("invalid hex byte in color: {s}"))?;
...
pub struct ThemeFile {
    #[serde(default)] pub editor:      EditorFile,
    #[serde(default)] pub scrollbar:   ScrollbarFile,
    #[serde(default)] pub statusbar:   StatusbarFile,
    #[serde(default)] pub popup:       PopupFile,
    #[serde(default)] pub syntax:      SyntaxFile,
    #[serde(default)] pub git:         GitFile,
    #[serde(default)] pub diagnostics: DiagnosticsFile,
    #[serde(default)] pub finder:      FinderFile,
    #[serde(default)] pub breadcrumb:  BreadcrumbFile,
    #[serde(default)] pub tabs:        TabsFile,
}
...
pub fn load_default() -> Theme {
    let file: ThemeFile = toml::from_str(DEFAULT_THEME_TOML)
        .expect("embedded default theme is malformed");
...
pub fn load_from(path: &Path) -> Theme {
    let default_file: ThemeFile = toml::from_str(DEFAULT_THEME_TOML)
        .expect("embedded default theme is malformed");
    let mut default_val: toml::Value = toml::from_str(DEFAULT_THEME_TOML)
        .expect("embedded default theme is malformed");
    ...
    fn try_from(f: ThemeFile) -> Result<Theme, String> {
        ...
        Ok(Theme {
            ...
            scrollbar: ScrollbarTheme {
                track: c!(f.scrollbar.track),
                thumb: c!(f.scrollbar.thumb),
            },
            ...
            popup: PopupTheme {
                bg:       c!(f.popup.bg),
                border:   c!(f.popup.border),
                hover:    c!(f.popup.hover),
                hover_fg: c!(f.popup.hover_fg),
                link:     c!(f.popup.link),
            },
            syntax: SyntaxTheme {
            ...
            tabs: TabsTheme {
                modified_dot: c!(f.tabs.modified_dot),
                active_fg:    c!(f.tabs.active_fg),
            },
        })
    }
}
