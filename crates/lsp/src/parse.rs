...
use super::types::{
    CompletionItem, DocumentSymbol, FileEdits, HoverSegment,
    LspDiagnostic, LspTextEdit, SemanticToken,
};
    ...
    if let Some(arr) = val.as_array() {
        ...
        if is_flat {
            for item in arr {
                ...
                if !name.is_empty() { out.push(DocumentSymbol { name, kind, start_line, end_line }); }
            }
        } else {
