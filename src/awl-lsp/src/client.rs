...
impl LspClient {
    pub(crate) fn start(key: &'static str, root: &Path) -> Option<Self> {
        ...
        let mut child = Command::new(cmd).args(args).stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped()).spawn().ok()?;
        ...
        thread::spawn(move || {
            reader_thread(
                ...
                pending_for_reader,
                hover_pending_reader,
                goto_pending_reader,
                rename_pending_reader,
                ca_pending_reader,
                comp_pending_reader,
                fmt_pending_reader,
                doc_sym_pending_reader,
            )
        });
    ...
    pub(crate) fn goto(&self, kind: GotoKind, path: &Path, line: u32, col: u32) {
        let Some(uri) = path_uri(path) else { return };
        let id = self.new_id();
        if let Ok(mut p) = self.goto_pending.lock() {
            p.insert(id, kind);
        }
    ...
    pub(crate) fn rename_symbol(&self, path: &Path, line: u32, col: u32, new_name: &str) {
        let Some(uri) = path_uri(path) else { return };
        let id = self.new_id();
        if let Ok(mut p) = self.rename_pending.lock() {
            p.insert(id, ());
        }
        send_raw(
            &self.writer_tx,
